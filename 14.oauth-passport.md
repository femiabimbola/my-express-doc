...
For Google, the url is `https://console.cloud.google.com/`

In the create credential origin
- the frontend url is the authorized javascript origin
- the authorized redirect url is the `${backend}/auth/google/callback`
-  You are to create the callback in the backend of your application
...
router.get('/api/auth/google', passport.authenticate('google',),)
router.get("/api/auth/google/callback",passport.authenticate('google'), signIn ); //The redirect setup on the oauth on google
```
